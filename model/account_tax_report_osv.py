    ...
    def _compute_report_balance(self, cr, uid, ids, reports, context=None):
        ...
        for report in reports:
            ...
            if report.type == 'accounts':
                ...
                res[report.id]['tax_code'] = self.browse(cr, uid, ids, context)._compute_account_balance(tax_codes)
                for value in res[report.id]['tax_code'].values():
                    for field in fields:
                        res[report.id][field] += value.get(field)
            elif report.type == 'account_type':
                # it's the sum of the tax codes
                res[report.id]['tax_code'] = self.browse(cr, uid, ids, context)._compute_account_balance(report.tax_ids)                
                for value in res[report.id]['tax_code'].values():
                    for field in fields:
                        res[report.id][field] += value.get(field)
            elif report.type == 'account_report' and report.account_report_id:
                # it's the amount of the linked report
                res2 = self.browse(cr, uid, ids, context)._compute_report_balance(report.account_report_id)
                for key, value in res2.items():
                    for field in fields:
                        res[report.id][field] += value[field]
            elif report.type == 'sum':
                # it's the sum of the children of this account.report
                res2 = self.browse(cr, uid, ids, context)._compute_report_balance(report.children_ids)
                for key, value in res2.items():
                    for field in fields:
                        report_obj = self.pool['account.tax.report'].browse(cr, uid, key, context)
