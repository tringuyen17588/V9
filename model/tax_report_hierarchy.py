...
class account_tax_report(models.Model):
    ...
    def open_print_report_wizard(self):
        wizard_form_id = self.env.ref('ia_au_gst_reporting.form_tax_excel_report_view').id
        return wizard_form_id
    ...
    children_ids = fields.One2many('account.tax.report', 'parent_id',
                                   'Account Report')
    ...
    tag_ids = fields.Many2many('account.account.tag', 'account_tax_report_tag',
                               'report_id', 'tag_id', 'Tax Groups')
    account_report_id = fields.Many2one('account.tax.report', 'Report Value')
    tax_ids = fields.Many2many('account.tax', 'account_tax_report_tax_code',
                               'report_id', 'tax_id', 'Tax Codes')
    sign = fields.Selection([(-1, 'Reverse balance sign'),
                             (1, 'Preserve balance sign')],
                            'Sign on Reports', required=True, default=1,
                            help='This sign will Decide whether report value'
                            ' to be added or deducted while calculating '
                            'Report Value of Parent Report'
                            ' e.g.: for sale type Tax Codes assign 1 and for '
                            'purchase type tax Codes assign -1 then while'
                            ' calculating report value of parent , sum of tax'
                            ' amounts of all purchase type tax codes will get'
                            ' deducted from sum of all sale type tax code ')
    ...
    style_overwrite = fields.Selection([(0, 'Automatic formatting'),
                               ...
                               help="You can set up here the format you want "
                               "this record to be displayed. If you leave the "
