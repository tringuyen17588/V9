...
try:
    ...
    raise Warning('This module has dependency on python-isoweek module!\n '
                  'You can install this module using command'
                  ' -sudo pip install isoweek')

from dateutil.relativedelta import relativedelta


class open_tax_journal_items(models.TransientModel):
    _name = 'open.tax.journal.items'
...
#             self.account_report_type = self.account_report_id.type
        ...
        if self.period_select:
            ...
            start_date_of_current_month = datetime.date(current_year,
                                                        current_month, 1)
            last_week_sunday = Week(current_year,
                                    current_week_number - 1).sunday()
            last_week_saturday = Week(current_year,
                                      current_week_number - 1).saturday()
            current_week_sunday = Week(current_year,
                                       current_week_number).sunday()
            current_week_saturday = Week(current_year,
                                         current_week_number).saturday()
            next_saturday = Week(current_year,
                                 current_week_number + 1).saturday()
            ...
            if self.period_select == 'all_dates':
                ...
                self.date_to = start_date_of_current_month + \
                ...
                days_of_last_month = monthrange(current_year,
                                                last_month_of_year)[1]
                last_day_of_year = account_config.fiscalyear_last_day
                last_day_date = datetime.date(current_year,
                                      last_month_of_year, last_day_of_year)
                first_day_date = last_day_date - relativedelta(months=12) +\
                    relativedelta(days=1)
                first_month_of_year = first_day_date.month
                first_day_of_year = first_day_date.day
#                 start_date_of_current_month = datetime.date(current_year,
#    current_month, first_day_of_year)
                for quarter in quarter_allocation:
                    month_list = quarter_allocation.get(quarter)
                    for r in range(1, 4):
                        if first_month_of_year > 12:
                            month_list.append({first_day_date.year + 1:
                                               first_month_of_year % 12})
                        else:
                            month_list.append({first_day_date.year:
                                               first_month_of_year})
                ...
                if quarter_allocation:
                    if self.period_select == 'tax_quarter_1':
                        ...
                        quarter_start_month = quarter_allocation[1][0].get(
                                                       quarter_start_year)
                        days_in_quarter_start_month = monthrange(
                                                     quarter_start_year,
                        ...
                        quarter_start_month = quarter_allocation[2][0].get(
                                                           quarter_start_year)
                        days_in_quarter_start_month = monthrange(
                                                     quarter_start_year,
                                                     quarter_start_month)[1]
                        ...
                        if days_in_quarter_start_month >= first_day_of_year:
                            ...
                            date_from = datetime.date(quarter_start_year,
                                                  ...
                                                  days_in_quarter_start_month)
                        self.date_from = date_from
                        self.date_to = date_from + relativedelta(months=3) -\
                            relativedelta(days=1)
                    elif self.period_select == 'tax_quarter_3':
                        quarter_start_year = quarter_allocation[3][0].keys()[0]
                        quarter_start_month = quarter_allocation[3][0].get(
                                                           quarter_start_year)
                        days_in_quarter_start_month = monthrange(
                                                     quarter_start_year,
                                                     quarter_start_month)[1]
                        ...
                        for q_no, months in quarter_allocation.iteritems():
                            ...
                            if current_month in m_list:
                                self.date_from = datetime.date(
                                               months[0].keys()[0],
                                               months[0].get(
                                                     months[0].keys()[0]),
                                               first_day_of_year)
                                self.date_to = datetime.date(
                                                 months[2].keys()[0],
                                                 months[2].get(
                                                       months[2].keys()[0]),
                                                 last_day_of_year)
                        ...
                        for q_no, months in quarter_allocation.iteritems():
                            ...
                            if current_month in m_list:
                                ...
                                self.date_from = datetime.date(
                                       ...
                                       months[2].get(
                                                     months[2].keys()[0]),
                                       first_day_of_year) - relativedelta(
                                                                      months=3)
                        ...
                        self.date_from = first_day_date - relativedelta(
                                                                    months=12)
